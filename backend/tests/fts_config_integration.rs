...
fn test_fts_config_integration() {
    ...
    // 模拟书签数据
    let bookmarks = vec![
        ("Linux内核开发指南", "深入讲解Linux内核的开发技术和调试方法", vec!["Linux".to_string(), "内核".to_string(), "技术".to_string()]),
        ("Rust编程语言", "Rust系统编程语言的核心特性", vec!["Rust".to_string(), "编程".to_string(), "语言".to_string()]),
        ("数据库设计与优化", "MySQL和PostgreSQL数据库性能优化", vec!["数据库".to_string(), "SQL".to_string(), "优化".to_string()]),
    ];
    ...
    for (i, (title, desc, tags)) in bookmarks.iter().enumerate() {
        ...
        // 验证关键词包含
        assert!(title_processed.contains("Linux") || title_processed.contains("Rust") || title_processed.contains("数据库"));
        assert!(tags_processed.contains("Linux") || tags_processed.contains("Rust") || tags_processed.contains("数据库"));
    }
...
fn test_fts_performance_considerations() {
    ...
    {
        // jieba 模式：允许更长的处理时间
        assert!(avg_time_per_item < 5.0, "jieba 模式平均处理时间过长: {:.3} ms", avg_time_per_item);
    }
    #[cfg(not(feature = "jieba"))]
    {
        // 默认模式：应该很快
        assert!(avg_time_per_item < 1.0, "默认模式平均处理时间过长: {:.3} ms", avg_time_per_item);
    }
