<template>
  <div class="container mx-auto px-4 py-8">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        ...
        <p class="text-muted-foreground">
          共 {{ bookmarksStore.pagination.total }} 个书签
        </p>
      </div>
    ...
    <!-- Bookmarks grid -->
    <div v-if="!bookmarksStore.isLoading && bookmarksStore.bookmarks.length > 0" class="grid gap-4">
      <div
      ...
      <!-- Load more -->
      <div v-if="bookmarksStore.pagination.has_more" class="text-center pt-4">
        <button
          @click="loadMore"
          :disabled="bookmarksStore.isLoading"
          class="px-4 py-2 text-sm border border-border rounded-md hover:bg-accent transition-colors disabled:opacity-50"
        >
          {{ bookmarksStore.isLoading ? '加载中...' : '加载更多' }}
        </button>
      </div>
    </div>

    <!-- Loading state -->
    <div v-else-if="bookmarksStore.isLoading" class="flex justify-center py-12">
